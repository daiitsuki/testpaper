...
import { CheckCircle2, ChevronLeft, FileText, User } from 'lucide-react';

export default function WrongNoteCreator() {
  const { studentId, examId } = useParams();
  const navigate = useNavigate();
  
  const student = useLiveQuery(() => db.students.get(Number(studentId)), [studentId]);
  ...
  useEffect(() => {
    if (student && exam) {
      setTitle(`${student.name} 학생의 오답노트 (${exam.title})`);
  ...
  const handleCreate = async () => {
    if (selectedIds.size === 0) return alert('최소 하나 이상의 틀린 문제를 선택하세요.');
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row justify-between items-end gap-6 mb-12">
        <div className="flex-1 space-y-2">
          ...
          <div className="flex items-center gap-2 text-slate-500">
            <User size={16} />
            <span>대상 학생: <span className="font-bold text-slate-900">{student.name}</span></span>
          </div>
        </div>
        <button 
      ...
      <div className="space-y-4">
        <h3 className="font-bold text-lg">틀린 문제를 선택하세요 ({selectedIds.size})</h3>
          ...
          {previews.map((img, idx) => (
            ...
            >
              <div className="aspect-[3/4] bg-slate-50 flex items-center justify-center p-2">
                <img src={img.url} alt="q" className="max-w-full max-h-full object-contain" />
              </div>
              <div className={`absolute top-3 left-3 w-8 h-8 rounded-full flex items-center justify-center font-bold shadow-sm ${selectedIds.has(img.id) ? 'bg-indigo-600 text-white' : 'bg-white text-slate-400'}`}>
                {idx + 1}
              </div>
              {selectedIds.has(img.id) && (
                <div className="absolute inset-0 bg-indigo-600/5 flex items-center justify-center">
                  <CheckCircle2 size={48} className="text-indigo-600 opacity-20" />
                </div>
              )}
            </div>
          ))}
